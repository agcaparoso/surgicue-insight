...
import { FormSelect } from '@/components/SiqComponents';
      ...
      <div className="space-y-8">
          ...
          <div className="grid grid-cols-3 gap-3">
            {sources.map(s => (
              <button
                ...
                className={`flex flex-col items-center justify-center p-4 rounded-lg border-2 transition-all ${source === s.key ? 'border-primary bg-primary/5' : 'border-border bg-card'}`}
              >
                <div className={source === s.key ? 'text-primary' : 'text-muted-foreground'}>{s.icon}</div>
                <span className={`text-[10px] font-bold uppercase mt-2 ${source === s.key ? 'text-primary' : 'text-muted-foreground'}`}>{s.label}</span>
              </button>
            ))}
          </div>
        </section>
        ...
        <section>
          ...
          <div
            ...
            className="border-2 border-dashed border-border rounded-2xl p-12 flex flex-col items-center justify-center text-center bg-card cursor-pointer hover:border-primary/50 transition-colors"
          >
        ...
        <section>
          ...
          <textarea
            className="w-full p-4 rounded-lg border border-border bg-card text-sm focus:outline-none focus:border-primary min-h-[100px]"
            placeholder="e.g. First attempt at burr hole placement, struggled with left edge alignment..."
          />
        </section>

        <div className="bg-card-alt p-4 rounded-lg flex gap-3 border border-border">
        ...
        <button
          ...
          className="w-full py-4 bg-primary text-primary-foreground rounded-lg font-bold shadow-soft hover:opacity-90 active:scale-[0.98] transition-all"
        >
      ...
      <AnimatePresence>
        {isAnalyzing && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-card z-[100] flex flex-col items-center justify-center p-8 text-center">
            ...
            <h3 className="text-2xl font-bold text-primary mb-6">Analyzing Footage</h3>
            <div className="space-y-3 w-full max-w-xs mx-auto">
              {[
                ...
                <div key={i} className="flex items-center gap-3 text-left">
                  <div className={`w-5 h-5 rounded-full flex items-center justify-center ${analysisStep > i ? 'bg-success' : analysisStep === i ? 'bg-primary' : 'bg-border'}`}>
                    {analysisStep > i && <CheckCircle2 size={12} className="text-success-foreground" />}
                    {analysisStep === i && <div className="w-1.5 h-1.5 bg-primary-foreground rounded-full animate-pulse" />}
                  </div>
